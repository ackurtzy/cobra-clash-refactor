        ...
        for row in range(height + 2):
            ...
            for col in range(width + 2):
                if row == 0 or col == 0 or row == height + 1 or col == width + 1:
                    ...
                    if board_pos in self.model.snake_one.locations:
                        if board_pos == self.model.snake_one.locations[0]:
                            line += self.snake_one_heads[
                                self.model.snake_one.directions[0]
                            ]
                        else:
                            line += self.snake_one_body
                    elif board_pos in self.model.snake_two.locations:
                        if board_pos == self.model.snake_two.locations[0]:
                            line += self.snake_two_heads[
                                self.model.snake_two.directions[0]
                            ]
                        else:
                            line += self.snake_two_body
                    elif board_pos in self.model.apples:
                        line += self.apple
                    else:
